...
class UserController extends Controller
{
    ...
    public function changePassword(Request $request)
    {
        ...
        if ($user->uid != null) {
            try {
                OptionalFirebase::auth()->changeUserPassword($user->uid, $password);
            } catch (\Exception $e) {
                Log::warning('Firebase password update skipped', ['error' => $e->getMessage()]);
            }
        }
    ...
    public function Edit(Request $request)
    {
        ...
        try {
            if ($user->uid != null) {
                try {
                    OptionalFirebase::auth()->updateUser($user->uid, [
                        'displayName' => $user->name,
                        'email' => $user->email,
                    ]);
                } catch (\Exception $e) {
                    Log::warning('Firebase user update skipped', ['error' => $e->getMessage()]);
                }
            }
