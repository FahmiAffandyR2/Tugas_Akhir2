...
class RouteController extends Controller
{
    ...
    public function createEdit(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'chosen_routes' => 'present|array|min:1',
            'ordered_directions' => 'present|array|min:1',
            'stops.*.address' => 'required|string',
            'stops.*.lat' => 'required|numeric',
            'stops.*.lng' => 'required|numeric',
        ], [
            'stops.min' => 'A route must contain at least two stops.',
            'chosen_routes.min' => 'A route must have a selected direction between every pair of stops.',
            'ordered_directions.min' => 'Directions must be calculated between every pair of stops.',
        ], []);
