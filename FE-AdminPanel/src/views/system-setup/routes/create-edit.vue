        ...
        saveRoute() {
            if (this.stops.length < 2) {
                this.$swal.fire({
                    ...
                    text: "Add at least two stops before saving the route.",
                    icon: "warning",
                    confirmButtonText: "OK",
                });
                return;
            ...
            const expectedSegments = this.stops.length - 1;
            const directionsComplete =
                this.ordered_directions.length === expectedSegments &&
                this.chosen_routes.length === expectedSegments &&
                this.ordered_directions.every(
                    (directions, index) =>
                        Array.isArray(directions) &&
                        directions.length > 0 &&
                        this.chosen_routes[index] !== undefined
                );

            if (!directionsComplete) {
                this.$swal.fire({
                    title: "Directions unavailable",
                    text: "Each pair of stops must have a calculated route. Check the backend Google Routes API key and try adding the stops again.",
                    icon: "warning",
                    confirmButtonText: "OK",
                });
                return;
